...
Deno.test("SettingsRepository.upsert inserts or updates and returns StoreSettings", async () => {
  const fakeDB: FakeDB = {
    queryObject: () =>
      Promise.resolve({ rows: [{ store_id: "s1", currency: "USD", checkout_rules_json: null }] }),
  };
  const repo = new SettingsRepository(fakeDB as unknown as Client);
...
Deno.test("SettingsRepository.findByStore returns row or null", async () => {
  const fakeSuccessDB: FakeDB = {
    queryObject: () => Promise.resolve({ rows: [{ store_id: "s2", currency: "EUR" }] }),
  };
