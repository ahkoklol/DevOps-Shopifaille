...
Deno.test("StoreRepository.create inserts and returns MerchantStore", async () => {
  ...
  const repo = new StoreRepository(fakeDB as unknown as Client);
  const result = await repo.create({ owner_user_id: "u1", name: "Store1", subdomain: "s1" });
...
Deno.test("StoreRepository.findById returns row or null", async () => {
  ...
  const repoEmpty = new StoreRepository(fakeEmptyDB as unknown as Client);
