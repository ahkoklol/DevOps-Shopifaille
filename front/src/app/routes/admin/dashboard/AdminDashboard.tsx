...
export default function AdminDashboard() {
  ...
  const goSection = (section: string) => navigate(`/admin/${shopId}/${section}`);

  const stats = [
    { label: 'Produits', value: products.length, icon: Package, color: 'blue', action: () => goSection('products') },
    { label: 'Commandes', value: orders.length, icon: ShoppingCart, color: 'green', action: () => goSection('orders') },
    { label: 'Catégories', value: categories.length, icon: TrendingUp, color: 'purple', action: () => goSection('categories') },
    { label: 'Clients', value: 156, icon: Users, color: 'orange', action: () => goSection('customers') },
  ];
